...
/**
 * Charge Google Maps JS API une seule fois via le bootstrap inline officiel.
 * Retourne une Promise qui resolve quand google.maps.importLibrary est prête.
 * Compatible avec plusieurs composants sur la même page.
 */
let _ready = null
function ensureGoogleMaps(apiKey) {
  ...
  _ready = new Promise((resolve, reject) => {
    // Si déjà chargé (ex: admin a déjà injecté le script)
    ...
    const triggerBootstrap = () =>
      bootstrapDone ||
      (bootstrapDone = new Promise(async (res, rej) => {
        ...
        d.__ib__ = res
        script.onerror = () => (bootstrapDone = rej(Error('Google Maps could not load.')))
        document.head.append(script)
      }))

    d.importLibrary = (name, ...args) =>
      libs.add(name) &&
      triggerBootstrap().then(() => d.importLibrary(name, ...args))

    // Déclenche le chargement initial
    d.importLibrary('maps').then(() => resolve()).catch(reject)
  })
...
export default function PublicNetworkMap({
  ...
  // Séparer les items avec/sans coordonnées
  const withGps    = items.filter(i => typeof i.lat === 'number' && typeof i.lng === 'number')
  const withoutGps = items.filter(i => i.lat == null || i.lng == null)

  useEffect(() => {
    if (!mapRef.current || !MAPS_API_KEY) {
      if (!MAPS_API_KEY) setError('Clé API Google Maps manquante.')
      setLoading(false)
      return
    }

    ensureGoogleMaps(MAPS_API_KEY)
